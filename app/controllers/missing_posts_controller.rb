class MissingPostsController < ApplicationController
  ...
  def index
    ...
    if @finished_missing_posts.count >= 5
      ...
      excess_missing_posts = @finished_missing_posts[5..-1]
  ...
  def solicitar
    ...
    if request.post?
      ...
      UserMailer.contact_by_email(modified_name, modified_lastname, modified_phone, modified_mail, dogname, dogmail).deliver_later
      ...
      AdminMailer.contact_by_email(user, @missing_post, modified_name, modified_lastname, modified_phone, modified_mail, dogname, omd_notifications_mail, dni).deliver_later

      mensaje= 'Solicitud de contacto exitosa, espere la respuesta!'
  ...
  private
    ...
    def missing_post_params
      params.require(:missing_post).permit(:name, :breed, :size, :gender, :description, :zone, :user_id, :finished, :confirmed_at, :age, :image)
    end
